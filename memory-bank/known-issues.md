...
2. **OPEN – `maxOperatingI` holds battery charge current for MPPT chargers.** Examples: `ss150_70` has opI 70 / maxIsc 50, `renogy_rover_60` has 60/50, and `fangpusun_flexmax_*` and `ss250_*_can` are similar (12 records where opI > maxIsc). `getCurrentClipLimit` prefers `maxOperatingI`, so the PV current check uses the battery-side amps. Fangpusun VT-65/80 have `maxIsc: 0`.
3. **OPEN – No charger power/charge-current check.** A 100/30 charger at 12 V caps at about 440 W, but `MaxDCPower` and `systemVoltage × charge current` are never compared with array Wp. This is the most common DIY/off-grid sizing mistake.
4. **OPEN – MPPT window unused.** `mpptRangeMin`/`mpptRangeMax` are never checked. Hot Vmp below the MPPT minimum and cold Vmp above the MPPT maximum are not flagged.
