...
5. **FIXED – Isc severity.** Isc above `maxIsc` is a separate check (`iscRating`) from clipping (Imp above operating current), and it is an error (decision D1) unless the controller sets `iscSelfLimiting`. Since 5507f6a, Isc above `maxIsc` is only a warning. Many datasheets treat max PV Isc as a hardware limit. Consider distinguishing Isc > maxIsc (error) from Imp > operating current (clipping warning).
