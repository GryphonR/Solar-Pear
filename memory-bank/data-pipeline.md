...
- `npm run verify:review-queue` shows review coverage (target: 80% of products with buy links) and what to review next. A review is recorded with `reviewed`, `reviewedAt`, `reviewedBy` and `notesReviewed`. The UI shows "Verified against datasheet" or "Specs not yet verified", and labels notes as AI-generated unless `notesReviewed` is set.
