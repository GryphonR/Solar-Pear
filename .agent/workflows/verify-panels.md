...
6.  **Correction & Safety Guards**:
    ...
    -   **Design Notes (`notes`)**: `notes` is a **per-series** field — see `SCHEMA.md`. If the panel belongs to a `panel-series`, update `notes` on **every** panel in that series to the same text (user-helpful information about the series as a whole: strengths/weaknesses, sentiment, application advice, and any notable power outputs within the series). If the panel has no `panel-series` (the `"(no series)"` edge case), update `notes` for that panel alone. If you keep a panel but have doubts (e.g. only one weak source), append to its notes: "Unverified: [brief reason]." so it can be manually reviewed later — for a series-shared note, append that caveat to the whole series' text, not just one bin.
